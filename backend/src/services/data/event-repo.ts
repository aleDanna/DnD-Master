...
export class EventRepository {
  ...
  async create(input: CreateEventInput): Promise<GameEvent> {
    const data = await db.insert<any>('events', {
      session_id: input.session_id,
      type: input.type,
      actor_id: input.actor_id || null,
      actor_name: input.actor_name || null,
      content: JSON.stringify(input.content),
      rule_citations: JSON.stringify(input.rule_citations || []),
      created_at: new Date(),
    });
  ...
  async getById(id: string): Promise<GameEvent | null> {
    const data = await db.findOne<any>('events', { id });
    if (!data) return null;
    return this.mapToEvent(data);
  }
  ...
  async listBySession(
    ...
    if (types && types.length > 0) {
      const typePlaceholders = types.map((_, i) => `$${paramIndex + i}`).join(', ');
      sql += ` AND type IN (${typePlaceholders})`;
      params.push(...types);
      paramIndex += types.length;
    }

    sql += ` ORDER BY sequence ASC LIMIT $${paramIndex}`;
    params.push(limit);
   ...
   */
  private mapToEvent(data: any): GameEvent {
    ...
    return {
      ...
      content: parseJson(data.content) as EventContent,
      rule_citations: parseJson(data.rule_citations) || [],
